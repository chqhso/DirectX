...
int KParser::_ParseObjectTM(KParsedMesh* pMesh, IXMLDOMNode* pNode){
	...
	try{
		...
		for (i = 0; i < nCount; i++){
			...
			Vector4 vec = _StrToVector4(str);
			pMesh->m_tmLocal.m[i][0] = vec.x;
			pMesh->m_tmLocal.m[i][1] = vec.y;
			pMesh->m_tmLocal.m[i][2] = vec.z;
			pMesh->m_tmLocal.m[i][3] = vec.w;
		}
		...
		for (i = 0; i < nCount; i++){
			...
			Vector4 vec = _StrToVector4(str);
			pMesh->m_tmWorld.m[i][0] = vec.x;
			pMesh->m_tmWorld.m[i][1] = vec.y;
			pMesh->m_tmWorld.m[i][2] = vec.z;
			pMesh->m_tmWorld.m[i][3] = vec.w;
		}
...
Color32 KParser::_StrToColor3(char str[]){
	Color32		val;
	char*		sep = " [],\n";
	char*		context;
