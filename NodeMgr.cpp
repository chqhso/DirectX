...
NodeMgr::NodeMgr(LPDIRECT3DDEVICE9 pDev, KParsedData* pData){
	...
	for (unsigned int i = 0; i < pData->m_meshList.size(); i++){
		if (!IsBone(i)){
...
NodeMgr::~NodeMgr(){
	...
	for (unsigned int i = 0; i < m_nodes.size(); i++) DEL(m_nodes[i]);
	stl_wipe_vector(m_nodes);
	for (unsigned int i = 0; i < m_tracks.size(); i++) DEL(m_tracks[i]);
...
void NodeMgr::Animate(std::vector<DTrack*> tracks, float fFrame){
	...
	for (int i = 0; i < size; i++){
		...
		Matrix4x4* pTM = m_nodes[id]->GetMatrixTMPtr();
...
void NodeMgr::CopyMesh(KParsedData* pData){
	...
	float fmin = Length(minBox);
	float fmax = Length(maxBox);
	m_fRadius = (fmin > fmax) ? fmin : fmax;
}
void NodeMgr::CheckBox(Vector3* minBox, Vector3* maxBox, Node& pNode){
	Vector3 min, max;
	...
	if (Length(*minBox) < Length(min)) *minBox = min;
	if (Length(*maxBox) < Length(max)) *maxBox = max;
}
