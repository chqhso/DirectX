...
int			QuadTree::_IsInFrustum(std::vector<CustomVertex>& pHeightMap, IFrustum& pFrustum){
	...
	pHeightMap[m_nCorner[0]].p.Copy(vec);
	b[0] = pFrustum.IsIn(&vec);
	pHeightMap[m_nCorner[1]].p.Copy(vec);
	b[1] = pFrustum.IsIn(&vec);
	pHeightMap[m_nCorner[2]].p.Copy(vec);
	b[2] = pFrustum.IsIn(&vec);
	pHeightMap[m_nCorner[3]].p.Copy(vec);
...
int			QuadTree::_GetLODLevel(std::vector<CustomVertex>& pHeightMap, Vector3* pCamera, float fLODRatio){
	Vector3 vec;
	pHeightMap[m_nCenter].p.Copy(vec);
	float d = Vector3::Distance(vec, *pCamera);
...
bool		QuadTree::_BuildQuadTree(std::vector<CustomVertex>& pHeightMap){
	if (_SubDivide()){
		// 좌측상단과, 우측하단의 거리를 구한다.
		Vector3 vec1, vec2;
		pHeightMap[m_nCorner[CORNER_TL]].p.Copy(vec1);
		pHeightMap[m_nCorner[CORNER_BR]].p.Copy(vec2);
		// 거리를 2로 나누어 반지름을 구한다.
		//m_fRadius = Vector3::Distance(vec2, vec1) / 2.0f;
		m_fRadius = Vector3::Length(vec1 - vec2) / 2.0f;
...
QuadTree*	QuadTree::_FindNode(std::vector<CustomVertex>& pHeightMap, int _0, int _1, int _2, int _3){
	...
	if (m_pChild[0]){
		...
		// 현재 맵상에서의 위치
		pt.x = (int)pHeightMap[n].p.vector.x;
		pt.y = (int)pHeightMap[n].p.vector.z;

		// 4개의 코너값을 기준으로 자식노드의 맵 점유범위를 얻는다.
		rc.SetRect(
			(int)pHeightMap[m_pChild[0]->m_nCorner[CORNER_TL]].p.vector.x,
			(int)pHeightMap[m_pChild[0]->m_nCorner[CORNER_TL]].p.vector.z,
			(int)pHeightMap[m_pChild[0]->m_nCorner[CORNER_BR]].p.vector.x,
			(int)pHeightMap[m_pChild[0]->m_nCorner[CORNER_BR]].p.vector.z);
		...
		// 4개의 코너값을 기준으로 자식노드의 맵 점유범위를 얻는다.
		rc.SetRect(
			(int)pHeightMap[m_pChild[1]->m_nCorner[CORNER_TL]].p.vector.x,
			(int)pHeightMap[m_pChild[1]->m_nCorner[CORNER_TL]].p.vector.z,
			(int)pHeightMap[m_pChild[1]->m_nCorner[CORNER_BR]].p.vector.x,
			(int)pHeightMap[m_pChild[1]->m_nCorner[CORNER_BR]].p.vector.z);
		// pt값이 점유범위안에 있다면 자식노드로 들어간다.
		if (rc.IsInRect(pt))
		...
		// 4개의 코너값을 기준으로 자식노드의 맵 점유범위를 얻는다.
		rc.SetRect(
			(int)pHeightMap[m_pChild[2]->m_nCorner[CORNER_TL]].p.vector.x,
			(int)pHeightMap[m_pChild[2]->m_nCorner[CORNER_TL]].p.vector.z,
			(int)pHeightMap[m_pChild[2]->m_nCorner[CORNER_BR]].p.vector.x,
			(int)pHeightMap[m_pChild[2]->m_nCorner[CORNER_BR]].p.vector.z);
		// pt값이 점유범위안에 있다면 자식노드로 들어간다.
		if (rc.IsInRect(pt))
		...
		// 4개의 코너값을 기준으로 자식노드의 맵 점유범위를 얻는다.
		rc.SetRect(
			(int)pHeightMap[m_pChild[3]->m_nCorner[CORNER_TL]].p.vector.x,
			(int)pHeightMap[m_pChild[3]->m_nCorner[CORNER_TL]].p.vector.z,
			(int)pHeightMap[m_pChild[3]->m_nCorner[CORNER_BR]].p.vector.x,
			(int)pHeightMap[m_pChild[3]->m_nCorner[CORNER_BR]].p.vector.z);
		// pt값이 점유범위안에 있다면 자식노드로 들어간다.
		if (rc.IsInRect(pt))
